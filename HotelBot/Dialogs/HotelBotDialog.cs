...
namespace HotelBot.Dialogs
{
	...
	public class HotelBotDialog
	{
		public static readonly IDialog<string> dialog = Chain.PostToChain()
			.Select(msg => msg.Text)
			.Switch(
			new RegexCase<IDialog<string>>(new Regex("^hi", System.Text.RegularExpressions.RegexOptions.IgnoreCase), (context, text) =>
			  {
		...
		private async static Task<IDialog<string>> AfterGreetingContinuation(IBotContext context, IAwaitable<object> item)
		{
			...
			return Chain.Return($"thank you hotel bot:{name}");
